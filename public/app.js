...
// 绑定原生 <video> 播放控件与 2D 源视频的精确双向同步 (进度条 Seeking / 播放 / 暂停)
function setupNativeControlsSync() {
  ...
  convertedVideo.addEventListener('seeking', () => {
    if (Math.abs(sourceVideo.currentTime - convertedVideo.currentTime) > 0.3) {
      sourceVideo.currentTime = convertedVideo.currentTime;
    }
  });
  ...
  try {
    ...
    if (mediaStream) {
      ...
      convertedVideo.play().then(() => {
        ...
        showToast('✨ 3D 视差视频流已挂载，支持原生进度条与全屏！');
      }).catch((e) => {
